...
readarray -t sampleSheets < <(find "${TMP_ROOT_DIR}/Samplesheets/POST_DRAGEN/" -mindepth 1 -maxdepth 1 -name "*.${SAMPLESHEET_EXT}" )
if [[ "${#sampleSheets[@]}" -eq '0' ]]
then
	log4Bash 'WARN' "${LINENO}" "${FUNCNAME[0]:-main}" '0' "No sample sheets found @ ${TMP_ROOT_DIR}/Samplesheets/POST_DRAGEN/: There is nothing to do."
	trap - EXIT
	exit 0
else
